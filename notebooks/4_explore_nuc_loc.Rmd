...
pwf_more_knots <- my_nullp(is_suc, bias.data=bias, nknots=19)
...
go_enriched_finalised <- go_enriched_finalised %>%
  filter(over_represented_adj_pval<0.1) %>%
  camprotR::remove_redundant_go() %>%
  camprotR::estimate_go_overrep(pwf_more_knots, go_human)

print(go_enriched_finalised)
```

OK, so we have two over-represented GO terms, the CC 'ribonucleoprotein complex' and the MF 'RNA binding'. Both are ~2-fold over-represented. 

```{r}
proteins_in_enriched_go <- go_human %>% filter(GO.ID %in% go_enriched_finalised$category) %>% pull(UNIPROTKB)
