...
```{r}
raw_psm$succ <- parse_PTM_scores(raw_psm$succ, ptm_col="ptmRS.Best.Site.Probabilities", threshold=75)
...
psm_res_imp_cm <- psm_res_imp %>% lapply(function(x){
  cm <- x %>%
    #log(base=2) %>%
    camprotR::center_normalise_to_ref(medians)
  #exprs(cm) <- 2^(exprs(cm))
  return(cm)
})
